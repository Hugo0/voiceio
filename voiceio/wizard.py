...
class Spinner:
    ...
    def set_message(self, message: str) -> None:
        """Update the spinner message live (for showing progress)."""
        self._message = message
...
def _toml_value(v) -> str:
    ...
    # Unknown scalar types round-trip as their string form.
    return f'"{_toml_escape(str(v))}"'


def _dump_toml(data: dict) -> str:
    """Serialize a 2-level config dict (top-level tables) back to TOML.

    Preserves every key present in ``data`` — including hand-edited and unknown
    keys — so re-running setup never drops user customizations.
    """
    lines: list[str] = ["# voiceio configuration — edit freely; 'voiceio setup' merges, never overwrites"]
...
def _load_raw_config() -> dict:
    """Load the existing config.toml as a raw nested dict (empty if absent)."""
    import tomllib
    try:
...
def _record_cloud_consent() -> None:
    """Record that the user opted into a cloud LLM feature.

    SECURITY SEAM: the security module owns the real consent ledger.
    """
    try:
        from voiceio.consent import record_consent  # type: ignore
        record_consent("autocorrect_cloud")
    except ImportError:
        pass  # TODO(security): consent.py not yet on this branch


def _secure_config_permissions() -> None:
    """Restrict config perms — it may hold cloud API keys.

    SECURITY SEAM: prefer the security module's helper when present on-branch.
    """
    try:
        from voiceio.consent import secure_file_permissions  # type: ignore
        secure_file_permissions(CONFIG_PATH)
        return
    except ImportError:
        pass  # TODO(security): consent.py not yet on this branch — safe fallback below
    if sys.platform != "win32":
        try:
            os.chmod(CONFIG_PATH, 0o600)
        except OSError:
            log.debug("Could not chmod config", exc_info=True)
...
def _write_config(
    ...
    """Merge wizard answers into the existing config, preserving everything else.

    Only the keys the wizard actually asks about are overwritten. Hand-edited
    keys, unknown keys, and secrets (e.g. a saved API key when the user didn't
    enter a new one) are preserved verbatim.
    """
    ...
    # Keys the wizard owns (asked about) → override only these.
    section("hotkey").update({"key": hotkey, "backend": backend})
    section("model").update({"name": model, "language": language})
    section("output").update({
        "method": method,
        "streaming": streaming,
        "punctuation_cleanup": punctuation_cleanup,
        "number_conversion": number_conversion,
        "voice_input_prefix": voice_input_prefix,
    })
    section("commands")["enabled"] = commands_enabled
    section("feedback").update({
        "sound_enabled": sound_enabled,
        "notify_clipboard": notify_clipboard,
    })
    section("tray")["enabled"] = tray_enabled
    section("llm").update({"enabled": llm_enabled, "model": llm_model})
    section("tts").update({
        "enabled": tts_enabled,
        "engine": tts_engine,
        "speed": tts_speed,
    ...
    # Autocorrect: only touch these keys when the wizard collected a NEW key
    # this run. Otherwise a saved secret / hand-edited endpoint is preserved.
    if autocorrect_api_key:
        section("autocorrect").update({
...
def _index_of(options: list[tuple[str, ...]], value: str, fallback: int = 0) -> int:
    """Index of the option whose first field == value (for current-value defaults)."""
...
def _current_note(is_rerun: bool, value: str) -> None:
    """Print a '(current: X)' line when re-running setup over an existing config."""
    if is_rerun and value != "":
        print(f"  {DIM}(current: {value}){RESET}")


def _preflight_deps(interactive: bool = True, install_deps: bool = False) -> None:
    """Detect missing system packages and offer a single install command.

    The #1 install funnel killer is a missing system dependency (a C toolchain
    for the evdev build, PortAudio, IBus + gir bindings). We surface one
    copy-pasteable command for THIS distro and, interactively, offer to run it.
    """
    from voiceio import platform as plat
    missing = plat.check_system_deps()
    if not missing:
        return

    cmd = plat.system_deps_install_cmd(missing)
    print(f"\n  {YELLOW}⚠  Missing system dependencies:{RESET}")
    for key in missing:
        print(f"    {RED}✗{RESET} {plat.dep_label(key)}")
    print(f"\n  {DIM}Install them all with:{RESET}")
    print(f"    {BOLD}{cmd}{RESET}\n")

    should_run = install_deps
    if interactive and not install_deps and cmd:
        should_run = _ask_yn(f"Run this now? ({cmd})", default=True)
    if not should_run or not cmd:
        if not interactive:
            print(f"  {DIM}Run the command above, then re-run setup.{RESET}")
        return

    print(f"\n  {CYAN}Running:{RESET} {cmd}\n")
    try:
        result = subprocess.run(cmd.split(), timeout=600)
        if result.returncode == 0:
            print(f"\n  {GREEN}✓{RESET} System dependencies installed.")
            # Some deps (evdev build) only take effect on a fresh Python process.
            still = plat.check_system_deps()
            if still:
                print(f"  {YELLOW}ℹ{RESET}  Re-run {BOLD}voiceio setup{RESET} to pick up: "
                      f"{', '.join(plat.dep_label(k) for k in still)}")
        else:
            print(f"\n  {RED}✗{RESET} Install failed (exit {result.returncode}). Run the command above manually.")
    except (FileNotFoundError, subprocess.TimeoutExpired) as e:
        print(f"\n  {RED}✗{RESET} Could not run installer: {e}")


def run_wizard() -> None:
    print(LOGO)

    is_rerun = CONFIG_PATH.exists()
    from voiceio.config import load as _load_cfg
    cur = _load_cfg() if is_rerun else None
    if is_rerun:
        print(f"  {DIM}Re-running setup — current values are shown as defaults.{RESET}")

    # ── Step 0: System-dependency preflight ─────────────────────────────
    _preflight_deps(interactive=True)
    ...
    _current_note(is_rerun, cur.model.language if cur else "")
    print()
    lang_default = _index_of(LANGUAGES, cur.model.language, 0) if cur else 0
    lang_idx = _ask_choice(LANGUAGES, default=lang_default)
    ...
    _current_note(is_rerun, cur.model.name if cur else "")
    print()
    model_default = _index_of(MODELS, cur.model.name, 2) if cur else 2
    model_idx = _ask_choice(MODELS, default=model_default)
    ...
    _current_note(is_rerun, cur.hotkey.key if cur else "")
    print()
    hotkey_options = [
    ...
    hk_default = _index_of(hotkey_options, cur.hotkey.key, 0) if cur else 0
    hk_idx = _ask_choice(hotkey_options, default=hk_default)
    ...
    if _ask_choice(advanced_options, default=0) == 1:
        ...
        voice_input_prefix = adv.get("voice_input_prefix", "")
    ...
    # ── Done ────────────────────────────────────────────────────────────
    _print_ready_panel(hotkey, autostart=(autostart_idx == 0))


_FIRST_SENTENCE = "Hello world, this is my first dictation with voiceio."


def _print_ready_panel(hotkey: str, autostart: bool) -> None:
    """The 'you're ready' panel shown after a successful setup."""
    ...
    if not _is_pipx_install() and not shutil.which("voiceio"):
        path_note = f"  {YELLOW}ℹ{RESET}  Restart your terminal so 'voiceio' is on PATH.\n\n"

    print(f"""
{GREEN}{'━' * 54}{RESET}
{BOLD}  You're ready.{RESET}  {DIM}Speak → text, locally and instantly.{RESET}

{path_note}{start_hint}

  {BOLD}Try it now:{RESET} press {BOLD}{hotkey}{RESET}, then say
    {CYAN}"{_FIRST_SENTENCE}"{RESET}
  {DIM}Press {hotkey} again to commit. Text streams at your cursor.{RESET}

  {DIM}Want the full tour?{RESET}  {CYAN}voiceio demo{RESET}
  {DIM}Other commands:{RESET}     {CYAN}voiceio doctor{RESET} · {CYAN}test{RESET} · {CYAN}correct{RESET} · {CYAN}history{RESET}

  {DIM}Your data lives in {CONFIG_DIR} and {LOG_DIR} —
  plain local files you own. Zero telemetry.{RESET}
  {DIM}Config: {CONFIG_PATH}   Logs: {LOG_PATH}{RESET}
{GREEN}{'━' * 54}{RESET}
""")
...
def _run_advanced_options(checks: dict, tray_ok: bool) -> dict:
    ...
    # Voice input prefix
    print(f"\n  {BOLD}Voice input prefix{RESET}")
    print(f"  {DIM}Prepend a tag (e.g. \"[voice]\") to every transcription. Helps{RESET}")
    print(f"  {DIM}readers/agents spot Whisper mishearings and infer intent.{RESET}\n")
    prefix_options = [
        ("Off", "default — no prefix"),
        ("[voice]", "tag every transcription with [voice]"),
        ("Custom", "type your own prefix"),
    ]
    prefix_idx = _ask_choice(prefix_options, default=0)
    if prefix_idx == 0:
        result["voice_input_prefix"] = ""
    elif prefix_idx == 1:
        result["voice_input_prefix"] = "[voice]"
    else:
        custom = _ask("Prefix", default="[voice]").strip()
        result["voice_input_prefix"] = custom

    return result


# ── Non-interactive setup (agents & scripts) ─────────────────────────────

_ALLOWED_ANSWER_KEYS = {
    "model", "language", "hotkey", "backend", "streaming",
    "tts_enabled", "tts_engine", "tts_speed",
    "llm_enabled", "llm_model",
    "autocorrect_api_key", "autocorrect_base_url", "autocorrect_model",
    "sound_enabled", "notify_clipboard", "commands_enabled",
    "punctuation_cleanup", "number_conversion", "voice_input_prefix",
    "tray_enabled", "install_service", "download_model", "install_system_deps",
}


def _progress(step: str, **fields) -> None:
    """Emit one machine-readable progress line to stdout."""
    parts = " ".join(f"{k}={v}" for k, v in fields.items())
    line = f"[voiceio-setup] step={step}"
    if parts:
        line += " " + parts
    print(line, flush=True)


def _fail(step: str, reason: str, code: int) -> int:
    _progress(step, status="error", reason=repr(reason))
    return code


def run_setup_noninteractive(answers: dict | None = None, *, install_deps: bool = False) -> int:
    """Non-interactive setup for agents/scripts. Returns a process exit code.

    Accepts ``answers`` (any subset of _ALLOWED_ANSWER_KEYS); unspecified values
    fall back to recommended defaults. Emits machine-readable ``[voiceio-setup]``
    progress lines and never reads stdin. Exit code 0 = success; non-zero with a
    printed reason otherwise.
    """
    answers = dict(answers or {})

    # ── Validate at the boundary ────────────────────────────────────────
    unknown = set(answers) - _ALLOWED_ANSWER_KEYS
    if unknown:
        return _fail("validate", f"unknown answer keys: {sorted(unknown)}", 2)
    valid_models = {m[0] for m in MODELS}
    model = str(answers.get("model", "small"))
    if model not in valid_models:
        return _fail("validate", f"invalid model '{model}'; choose from {sorted(valid_models)}", 2)
    hotkey = str(answers.get("hotkey", "ctrl+alt+v")).strip()
    if not hotkey:
        return _fail("validate", "hotkey must not be empty", 2)
    language = str(answers.get("language", "en")).strip() or "en"

    _progress("start", mode="noninteractive", model=model, language=language, hotkey=hotkey)
    ...
    if missing:
        cmd = plat.system_deps_install_cmd(missing)
        _progress("preflight", status="missing", deps=",".join(missing), cmd=repr(cmd))
        if install_deps or answers.get("install_system_deps"):
            _progress("preflight", status="installing")
            try:
                rc = subprocess.run(cmd.split(), timeout=600).returncode
            except (FileNotFoundError, subprocess.TimeoutExpired) as e:
                return _fail("preflight", f"installer failed: {e}", 4)
            if rc != 0:
                return _fail("preflight", f"installer exit {rc}", 4)
            missing = plat.check_system_deps()
    if not missing:
    ...
    if "tray_enabled" in answers:
        tray_ok = bool(answers["tray_enabled"])

    ac_key = str(answers.get("autocorrect_api_key", ""))
    if ac_key:
        _record_cloud_consent()

    # ── Write config (merge-preserving) ─────────────────────────────────
    _write_config(
        model=model, language=language, hotkey=hotkey, method="auto",
        streaming=bool(answers.get("streaming", True)), backend=backend,
        sound_enabled=bool(answers.get("sound_enabled", True)),
        notify_clipboard=bool(answers.get("notify_clipboard", False)),
        tray_enabled=tray_ok,
        commands_enabled=bool(answers.get("commands_enabled", True)),
        punctuation_cleanup=bool(answers.get("punctuation_cleanup", True)),
        number_conversion=bool(answers.get("number_conversion", True)),
        voice_input_prefix=str(answers.get("voice_input_prefix", "")),
        llm_enabled=bool(answers.get("llm_enabled", False)),
        llm_model=str(answers.get("llm_model", "")),
        autocorrect_api_key=ac_key,
        autocorrect_base_url=str(answers.get("autocorrect_base_url", "")),
        autocorrect_model=str(answers.get("autocorrect_model", "")),
        tts_enabled=bool(answers.get("tts_enabled", True)),
        tts_engine=str(answers.get("tts_engine", "auto")),
        tts_speed=float(answers.get("tts_speed", 1.0)),
        quiet=True,
    )
    _progress("config", status="written", path=str(CONFIG_PATH))

    # ── IBus engine component (best-effort) ─────────────────────────────
    if checks["is_linux"] and checks["ibus"] and checks["ibus_gi"]:
        try:
            from voiceio.typers.ibus import _ensure_gnome_input_source, install_component
            if install_component():
                _ensure_gnome_input_source()
                _progress("ibus", status="installed")
        except Exception as e:  # noqa: BLE001 — best-effort, never fatal
            _progress("ibus", status="skipped", reason=repr(str(e)))

    # ── GNOME shortcut for socket backend ───────────────────────────────
    if checks["is_linux"] and backend == "socket":
        desktop = os.environ.get("XDG_CURRENT_DESKTOP", "")
        if "GNOME" in desktop:
            ok = _setup_gnome_shortcut(hotkey)
            _progress("shortcut", status="ok" if ok else "manual", hotkey=hotkey)
        else:
            _progress("shortcut", status="manual", hint="bind 'voiceio toggle' in your WM")
    ...
    if answers.get("install_service", True) and system_error is None:
        ...
        elif checks["is_linux"] and has_systemd():
            autostart = install_service()
        _progress("service", status="installed" if autostart else "skipped")
        if autostart and checks["is_linux"]:
            try:
                subprocess.run(["systemctl", "--user", "restart", "voiceio.service"],
                               capture_output=True, timeout=5)
            except (FileNotFoundError, subprocess.TimeoutExpired):
                pass
